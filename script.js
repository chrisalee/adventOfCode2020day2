...
//use a regular expression because there is a pattern with the data given,  //independent of javascript
//dense strings that fall into patterns  (digits hyphencharacter digits space word character colon characterphrase)
...
const result = 
  list
    .filter((entry) => {
      ...
      // return {doesItPass: charCount >= parseInt(min) && charCount <= parseInt(max), password: password}
      // return [min, max, targetChar, password, charCount];
    }).length;

// console.log(result);

const password2 = list.filter((entry) => {
  const [_, position1, position2, targetChar, password] = entry.match(pattern);
  const charPos1 = [...password][parseInt(position1) - 1];
  const charPos2 = [...password][parseInt(position2) - 1];
  
  if(charPos1 === charPos2) {
    return false;
  } else if(charPos1 === targetChar || charPos2 === targetChar) {
    return true;
  }
